...
function init() {
  ...
  $.post("../ajax/articulo.php?op=selectFamilia&idempresa=" + $idempresa, function (r) {
    $("#idfamilia").html(r);
  ...
  $.post("../ajax/articulo.php?op=selectUnidad&idempresa=" + $idempresa, function (r) {
...
function listar() {
  var $idempresa = $("#idempresa").val();
  var $idalmacen = $(".js-example-placeholder-single.js-states").val();
  ...
  tabla = $('#tbllistado').dataTable(
    {
      ...
      "ajax":
      {
        url: '../ajax/articulo.php?op=listar&idempresa=' + $idempresa + '&idalmacen=' + $idalmacen,
        type: "get",
        dataType: "json",
        error: function (e) {
...
function generarCodigoAutomatico() {
  ...
  if ($('#generar-cod-correlativo').prop('checked') && $('#codigo').val() === '') {
    $.getJSON('../ajax/articulo.php?action=GenerarCodigo&idempresa=' + idempresa, function (data) {
      console.log('Respuesta del servidor:', data);
      $('#codigo').val(data.codigo);
      setCodigoFieldReadonly();
    });
...
function setCodigoFieldReadonly() {
  console.log('Campo código se estableció como solo lectura.');
...
$(document).ready(function () {
  var $idempresa = $("#idempresa").val();
  $.ajax({
    url: '../ajax/ajaxstock.php?action=listarAlmacen&idempresa=' + $idempresa,
    type: 'GET',
    dataType: 'json',
...
$(".js-example-placeholder-single.js-states").on("change", function (e) {
  ...
  tabla.ajax.url('../ajax/articulo.php?op=listar&idempresa=' + idempresa + '&idalmacen=' + idalmacen).load();
