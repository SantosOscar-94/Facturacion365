...
function init() {
  ...
  mostrarform(false);
  verificarCargoYMostrarNotaPedidoPorUsuario();
}
...
function listar() {
  tabla = $("#tbllistado")
    .dataTable({
      ...
      ajax: {
        url: "../ajax/notapedido.php?op=listar" + "&idusuario=" + sessionStorage.getItem("idusuario"),
        type: "get",
        dataType: "json",
        error: function (e) {
...
// Esta función verifica si el cargo en sessionStorage es 1 y luego ejecuta ListarEnvioAutomatico
function verificarCargoYMostrarNotaPedidoPorUsuario() {
...
function listarTodo() {
  tabla = $("#tbllistado")
    .dataTable({
      ...
      ajax: {
        url: "../ajax/notapedido.php?op=listarTodo",
        type: "get",
        dataType: "json",
        error: function (e) {
          console.log(e.responseText);
...
function verificarCargoYMostrarNotasPedido() {
  const cargo = sessionStorage.getItem('cargo');
  if (cargo === '0') {
    listarTodo();
  } else {
...
verificarCargoYMostrarNotasPedido();
...
function listarArticulos() {
  ...
  tablaArti = $("#tblarticulos")
    .dataTable({
      ...
      ajax: {
        url:
          "../ajax/notapedido.php?op=listarArticulonota&tprecio=" +
          $tipoprecio +
          "&tb=" +
          tpn +
          "&itm=" +
...
function actualizarNum(e) {
  ...
  $.post(
    "../ajax/notapedido.php?op=actualizarNumero&Num=" +
    numero +
    "&Idnumeracion=" +
    idnumeracion,
    function (r) { }
...
function mostrar(idboleta) {
  $.post(
    "../ajax/notapedido.php?op=mostrar",
    { idboleta: idboleta },
...
function agregarArticuloxCodigo(e) {
  ...
  if (e.keyCode === 13 && !e.shiftKey) {
    $.post(
      "../ajax/notapedido.php?op=listarArticulosboletaxcodigo&codigob=" +
      codigob,
      function (data, status) {
        data = JSON.parse(data);
