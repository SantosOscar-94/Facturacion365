...
//Guardar en tabla envicorreo =========================================
    ...
    public function AutocompletarRuc($buscar){
      ...
      $result = mysqli_query($connect, $sql); // Cambio aquí
    
      if ($result->num_rows > 0) {
        while ($fila = $result->fetch_array()) { // Cambio aquí
...
public function listarnumerofilas($idcotizacion)
...

  {
    ...
    $sql="select c.idcotizacion, e.tipoimpresion from cotizacion c inner join empresa e on c.idempresa=e.idempresa  where e.idempresa='$idempresa'  order by idcotizacion desc limit 1";
