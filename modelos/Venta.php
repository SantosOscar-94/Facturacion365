...
class Venta
{
  ...
  (
    select 
        ...
        f.tipo_documento_07, 
        f.fecha_emision_01,
        case when f.estado = '5' then f.total_operaciones_gravadas_monto_18_2 else 0 end as subtotal,  
        f.numeracion_08, 
        case when f.estado = '5' then f.sumatoria_igv_22_1 else 0 end as igv,
        case when f.estado = '5' then f.importe_total_venta_27 else 0 end as total,  
        f.estado, 
        p.numero_documento, 
        p.razon_social, 
        f.icbper,
        f.tipofactura, 
        f.tipo_moneda_28 as tipomoneda, 
        f.tcambio,
        case when f.estado = '5' then f.efectivo else 0 end as efectivo,
        case when f.estado = '5' then f.visa else 0 end as visa,
        case when f.estado = '5' then f.yape else 0 end as yape,
        case when f.estado = '5' then f.plin else 0 end as plin,
    ...
    where 
        ...
        and day(f.fecha_emision_01) = '$dia'
    ...
    union all
        ...
        b.fecha_emision_01,
        CASE WHEN b.estado = '5' THEN b.monto_15_2 ELSE 0 END as subtotal,  
        b.numeracion_07, 
        CASE WHEN b.estado = '5' THEN b.sumatoria_igv_18_1 ELSE 0 END as igv,
        CASE WHEN b.estado = '5' THEN b.importe_total_23 ELSE 0 END as total,
        b.estado, 
        p.numero_documento, 
        p.razon_social, 
        b.icbper,
        b.tipoboleta as tipofactura, 
        b.tipo_moneda_24 as tipomoneda, 
        b.tcambio,
        case when b.estado = '5' then b.efectivo else 0 end as efectivo,
        case when b.estado = '5' then b.visa else 0 end as visa,
        case when b.estado = '5' then b.yape else 0 end as yape,
        case when b.estado = '5' then b.plin else 0 end as plin,
    ...
    where 
        ...
        and day(np.fecha_emision_01) = '$dia'
        and p.tipo_persona = 'cliente' 
    ...
    select 
        ...
        ncd.fecha,
        case when ncd.estado = '5' then if(ncd.codigo_nota='07', ncd.total_val_venta_og * -1, ncd.total_val_venta_og) else 0 end as subtotal,  
        ncd.numeroserienota, 
        case when ncd.estado = '5' then if(ncd.codigo_nota='07', ncd.sum_igv * -1, ncd.sum_igv) else 0 end as igv,
        case when ncd.estado = '5' then if(ncd.codigo_nota='07',ncd.importe_total * -1,ncd.importe_total) else 0 end as total, 
        ncd.estado, 
        p.numero_documento, 
        p.razon_social, 
        case when ncd.estado = '5' then ncd.icbper else 0 end as icbper,
  ...
  public function resumend($ano, $mes, $dia, $idempresa, $idusuario)
  {
    $sql = "
    SELECT 
    idboleta as id,
...
FROM 
    ...
    WHERE 
        YEAR(fecha_baja)='$ano' 
        AND MONTH(fecha_baja)='$mes' 
        AND DAY(fecha_baja)='$dia' 
        AND b.estado='3' 
        AND e.idempresa='$idempresa'
...
as tabla ";
  ...
  //FILTRO por fecha
  public function filtrofechaReportes($fecha_desde, $fecha_hasta, $idempresa, $tmon)
  {
...
(
  ...
  left join 
      detalle_fac_art dfa on f.idfactura = dfa.idfactura
  where 
      f.fecha_emision_01 between '$fecha_desde' and '$fecha_hasta'
      and p.tipo_persona = 'cliente' 
      and f.estado in ('5', '3', '0', '6', '1', '4')  
      and e.idempresa = '$idempresa' 
      and f.tipo_moneda_28 = '$tmon'
...
left join 
  detalle_boleta_producto dbp on b.idboleta = dbp.idboleta
where 
  b.fecha_emision_01 between '$fecha_desde' and '$fecha_hasta'
  and p.tipo_persona = 'cliente' 
  and b.estado in ('5', '3', '0', '6', '1', '4')  
  and e.idempresa = '$idempresa' 
  and b.tipo_moneda_24 = '$tmon'
...
left join 
  detalle_notapedido_producto dnpp on np.idboleta = dnpp.idboleta
where 
  np.fecha_emision_01 between '$fecha_desde' and '$fecha_hasta'
  and p.tipo_persona = 'cliente' 
  and np.estado in ('5', '3', '0', '6', '1', '4')  
  and e.idempresa = '$idempresa' 
  and np.tipo_moneda_24 = '$tmon'
...
inner join 
  empresa e on ncd.idempresa = e.idempresa 
where 
  ncd.fecha between '$fecha_desde' and '$fecha_hasta'
  and p.tipo_persona = 'cliente' 
  and ncd.estado in ('5', '3', '0', '1', '4')  
  and e.idempresa = '$idempresa' 
  and ncd.tipo_moneda = '$tmon'
...
order by fecha asc;";
  ...
  public function reporteBoletayFacturasTributario($FechaDesdeIni, $FechaHastaFin, $idempresa, $tmonedaa)
  {
    $sql = "select
    fecha_emision_01 as fecha,
    case tipo_documento_06
        when '01' then 'FACTURA ELECTRÓNICA'
        when '03' then 'BOLETA ELECTRÓNICA'
        else tipo_documento_06
    end as 'TIPO CPE',
    numeracion_07 as 'SERIE Y NUMERO',
    rucCliente as 'RUC/DNI',
    RazonSocial as 'RAZON SOCIAL/NOMBRE',
    case when codigo_tributo_18_3 = '9997' then 0 else monto_15_2 end as 'op. gravadas',
    0 as 'op. gratuitas',
    case when codigo_tributo_18_3 = '9997' then importe_total_23 else 0 end as 'op. exoneradas',
    0 as 'op. inafectas',
    tdescuento as 'total dscto',
    sumatoria_igv_18_2 as igv,
    0 as 'imp. bolsa',
    importe_total_23 as total,
    'habilitado' as estado,
    detallesunat as 'respuesta sunat',
    descripcion_leyenda_26_2 as observacion
    from boleta
    where
    idempresa = '$idempresa'
    and tipo_moneda_24 = '$tmonedaa'
    and fecha_emision_01 between '$FechaDesdeIni' and '$FechaHastaFin'

    union

    select
    fecha_emision_01 as fecha,
    case tipo_documento_07
        when '01' then 'FACTURA ELECTRÓNICA'
        when '03' then 'BOLETA ELECTRÓNICA'
        else tipo_documento_07
    end as 'TIPO CPE',
    numeracion_08 as 'SERIE Y NUMERO',
    rucCliente as 'RUC/DNI',
    RazonSocial as 'RAZON SOCIAL/NOMBRE',
    case when total_operaciones_gravadas_codigo_18_1 = '9997' then 0 else total_operaciones_gravadas_monto_18_2 end as 'op. gravadas',
    0 as 'op. gratuitas',
    case when total_operaciones_gravadas_codigo_18_1 = '9997' then importe_total_venta_27 else 0 end as 'op. exoneradas',
    0 as 'op. inafectas',
    tdescuento as 'total dscto',
    sumatoria_igv_22_1 as igv,
    0 as 'imp. bolsa',
    ...
    'habilitado' as estado,
    detallesunat as 'respuesta sunat',
    descripcion_leyenda_31_2 as observacion
      from factura
      where
    idempresa = ' $idempresa'
    and tipo_moneda_28 = '$tmonedaa'
    and fecha_emision_01 between '$FechaDesdeIni' and '$FechaHastaFin';";
    return ejecutarConsulta($sql);
  }
...
where year(fecha_baja)='$ano' and month(fecha_baja)='$mes' and day(fecha_baja)='$dia' and b.estado='3'
  ...
  public function listarcomprobantes($fec1, $fec2, $tipocte, $Estd, $idusuario)
  {
...
FROM (
    ...
    WHERE
        ...
        AND f.tipo_documento_07 = '$tcFactura'
        AND f.estado IN ($Estd)
        AND u.idusuario = '$idusuario' -- Agregar condición para idusuario
    ...
    WHERE
        DATE_FORMAT(fecha_emision_01, '%Y-%m-%d') BETWEEN '$fec1' AND '$fec2'
        AND b.tipo_documento_06 = '$tcBoleta'
        AND b.estado IN ($Estd)
        AND u.idusuario = '$idusuario' -- Agregar condición para idusuario
        AND (u.cargo = 0 OR (u.cargo = 1 AND b.idusuario = '$idusuario')) -- Agregar condición para cargo
) AS estados";
    return ejecutarConsulta($sql);
