...
class ModeloStock
{
    ...
    /*   public function totalAlmacenActiva($nombre, $estado, $idempresa)
    {
        $sql = "select idalmacen, nombre, estado from almacen where estado = 1 and idempresa = '$idempresa'";
        return ejecutarConsulta($sql);
    }  */

    public function totalAlmacenActiva($nombre, $estado, $idempresa)
{
    $sql = "SELECT idalmacen, nombre, estado, idempresa FROM almacen WHERE estado = 1 AND idempresa = '$idempresa'";
    return ejecutarConsulta($sql);
}
