...
class ModeloTransferencia
{
    ...
    public function insertar($idusuario, $fecha_emision_01, $firma_digital_36, $idempresa, $tipo_documento_06, $numeracion_07, $idcl, $codigo_tipo_15_1, $monto_15_2, $sumatoria_igv_18_1, $sumatoria_igv_18_2, $sumatoria_igv_18_3, $sumatoria_igv_18_4, $sumatoria_igv_18_5, $importe_total_23, $codigo_leyenda_26_1, $descripcion_leyenda_26_2, $tipo_documento_25_1, $guia_remision_25, $version_ubl_37, $version_estructura_38, $tipo_moneda_24, $tasa_igv, $idarticulo, $numero_orden_item_29, $cantidad_item_12, $codigo_precio_14_1, $precio_unitario, $igvBD, $igvBD5, $afectacion_igv_3, $afectacion_igv_4, $afectacion_igv_5, $afectacion_igv_6, $igvBD2, $vvu, $subtotalBD, $codigo, $unidad_medida, $idserie, $SerieReal, $numero_boleta, $tipodocuCliente, $rucCliente, $RazonSocial, $hora, $descdet, $vendedorsitio, $idnota, $tiponota, $transferencia, $cantidadreal, $faltante, $adelanto, $ncotizacion, $ambtra, $efectivo, $visa, $yape, $plin, $mastercard, $deposito)
    {
        ...
        detalle_transferenciastock_producto(idboleta,
          idarticulo,
          numero_orden_item_29,
          cantidad_item_12,
          codigo_precio_14_1,
    ...
    { 
        ...
        $query = "select idboleta, idarticulo  from detalle_transferenciastock_producto where idboleta='$idboleta'";
     ...
     (idcomprobante,
      ...
      costo_2,valor_final)
            ...
            (select a.idarticulo from articulo a inner join detalle_transferenciastock_producto dtb on a.idarticulo=dtb.idarticulo where a.idarticulo='$Ida[$i]' and dtb.idboleta = '$Idb[$i]'),

            'ANULADO',

            (select a.codigo from articulo a inner join detalle_transferenciastock_producto dtb on a.idarticulo=dtb.idarticulo where a.idarticulo='$Ida[$i]' and dtb.idboleta = '$Idb[$i]'),
    ...
    public function baja($idnotap, $fecha_baja, $com, $hora)
    {
        ...
        $query = "select idboleta, idarticulo  from detalle_transferenciastock_producto where idboleta='$idnotap'";
        $resultado = mysqli_query($connect, $query);
        $Idb = array();
        $Ida = array();
        while ($fila = mysqli_fetch_assoc($resultado)) {
     ...
     (idcomprobante,
      ...
      costo_2,valor_final)
            ...
            (select a.codigo from articulo a inner join detalle_transferenciastock_producto dtb on a.idarticulo=dtb.idarticulo where a.idarticulo='$Ida[$i]' and dtb.idboleta = '$Idb[$i]'),
             ...
             (select fecha_emision_01 from transferenciastock where idboleta='$Idb[$i]'),
             '14',
    ...
    public function mostrar($idboleta)
    {
        ...
        b.estado
        from
        transferenciastock b inner join persona p on b.idcliente=p.idpersona inner join usuario u on b.idusuario=u.idusuario WHERE b.idboleta='$idboleta'";
   ...
   public function listar()
   {
    $sql = "select
        ...
        t.adelanto,
        t.faltante,
        t.descripcion_leyenda_26_2 AS motivo,
        format(t.importe_total_23, 2) as importe_total_23,
        t.estado,
        p.nombres,
        p.apellidos,
        e.numero_ruc,
    ...
    public function ventacabecera($idboleta)
    {
        ...
        from
        transferenciastock np inner join persona p on np.idcliente=p.idpersona
    ...
    {
        $sql = "select  numeracion_07, importe_total_23 as total from transferenciastock np inner join persona p on np.idcliente=p.idpersona where p.idpersona='$idcliente' and np.estado='1'";
        return ejecutarConsulta($sql);
    }
    ...
    public function ventadetalle($idboleta)
    {
        ...
        um.abre
        from
        detalle_transferenciastock_producto db inner join articulo a on db.idarticulo=a.idarticulo inner join umedida um on a.unidad_medida=um.idunidad
        where
    ...
    {
        $sw = true;
        $sqlestado = "update transferenciastock set estado='$st' where idboleta='$idboleta'";
        ejecutarConsulta($sqlestado) or $sw = false;
        return $sw;
    }
    ...
    public function listarcomprobantes($dnicliente)
    {
        ...
        from
        transferenciastock n inner join persona p on n.idcliente=p.idpersona
    ...
    public function actualizarestados($idnota, $cestado)
    {
        ...
        while ($num_elementos < count($idnota)) {
            //Guardar en Detalle
            $sql = "update transferenciastock set estado='$cestado' where idboleta= '$idnota[$num_elementos]'";
    ...
    {
        $sql = "select np.idboleta, e.tipoimpresion from transferenciastock np inner join empresa e on np.idempresa=e.idempresa  where e.idempresa='$idempresa'  order by idboleta desc limit 1";
        return ejecutarConsultaSimpleFila($sql);
    }
