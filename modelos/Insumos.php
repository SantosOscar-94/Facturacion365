...
Class Insumos
{
    ...
    public function insertar($tipodato, $idsaldoini, $fecharegistro, $categoriai, $documnIDE, $numDOCIDE, $acredor, $descripcion, $monto, $idusuario)
    {
        if ($tipodato == 'gasto') {
            $idsaldoini = $this->traeridsaldoiniinusmos($idusuario);
            $sql = "INSERT INTO insumos (tipodato, idsaldoini, idcategoriai, fecharegistro, descripcion, gasto, documnIDE, numDOCIDE, acredor, idusuario)
                    VALUES ('$tipodato', '$idsaldoini', '$categoriai', '$fecharegistro', '$descripcion', '$monto', '$documnIDE', '$numDOCIDE', '$acredor', '$idusuario')";
        } else {
            $idsaldoini = $this->traeridsaldoiniinusmos($idusuario);
            $sql = "INSERT INTO insumos (tipodato, idsaldoini, idcategoriai, fecharegistro, descripcion, ingreso, documnIDE, numDOCIDE, acredor, idusuario)
                    VALUES ('$tipodato', '$idsaldoini', '$categoriai', '$fecharegistro', '$descripcion', '$monto', '$documnIDE', '$numDOCIDE', '$acredor', '$idusuario')";
        }
	...
	public function listar($fecha, $idusuario)
	{
		$sql="select * from insumos ins inner join categoriainsumos ci on ins.idcategoriai=ci.idcategoriai where
        date(fecharegistro)='$fecha' and idusuario ='$idusuario' order by idinsumo desc";
		return ejecutarConsulta($sql);		
	}
	...
	{
		$sql="select ci.descripcionc,   ins.descripcion, ins.acredor, gasto as totalgd from insumos ins inner join categoriainsumos ci on ins.idcategoriai=ci.idcategoriai  where
	...
	{
		$sql="select ci.descripcionc,   ins.descripcion, ins.acredor, ingreso as totalgd from insumos ins inner join categoriainsumos ci on ins.idcategoriai=ci.idcategoriai  where
