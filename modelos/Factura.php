...
class Factura
{
  ...
  public function editarCaja($idcaja, $fecha, $montoi, $montof, $st, $idempresa)
  {
    ...
    //Implementamos un método para insertar registros para factrura
    public function traeridsaldoinifac($idusuario){
  ...
  public function insertar($idusuario, $idsaldoini, $fecha_emision, $firma_digital, $idempresa, $tipo_documento, $numeracion, $idcliente, $total_operaciones_gravadas_codigo, $total_operaciones_gravadas_monto, $sumatoria_igv_1, $sumatoria_igv_2, $codigo_tributo_3, $nombre_tributo_4, $codigo_internacional_5, $importe_total_venta, $tipo_documento_guia, $guia_remision_29_2, $codigo_leyenda_1, $descripcion_leyenda_2, $version_ubl, $version_estructura, $tipo_moneda, $tasa_igv, $idarticulo, $numero_orden_item, $cantidad, $codigo_precio, $pvt, $igvBD2, $igvBD3, $afectacion_igv_3, $afectacion_igv_4, $afectacion_igv_5, $afectacion_igv_6, $igvBD, $valor_unitario, $subtotalBD, $codigo, $unidad_medida, $idserie, $SerieReal, $numero_factura, $tipodocuCliente, $rucCliente, $RazonSocial, $hora, $sumadcto, $vendedorsitio, $email, $domicilio_fiscal2, $codigotributo, $tdescuento, $tcambio, $tipopago, $nroreferencia, $ipagado, $saldo, $descdet, $total_icbper, $tipofactura, $cantidadreal, $idcotizacion, $ccuotas, $fechavecredito, $montocuota, $otroscargos, $tadc, $transferencia, $ncuotahiden, $montocuotacre, $fechapago, $fechavenc, $efectivo, $visa, $yape, $plin, $mastercard, $deposito)
  {
    ...
    $idsaldoini = $this->traeridsaldoinifac($idusuario);
    if (!$idsaldoini) {
...
//$Factura=$pdf->Output($regv->numeracion_08.'.pdf','I');
  ...
  public function generarxml($idfactura, $idempresa)
  {
    ...
    while ($row = mysqli_fetch_assoc($result)) {
      ...
      // Redondea el total a pagar y los montos de las cuotas a dos decimales
        $totalRedondeado = round($total, 2);
        $montosCuotasRedondeados = array_map(function($monto) {
            return round($monto, 2);
        }, $montocuotacredito);

        // Verifica si la suma de las cuotas coincide con el total redondeado
        if (array_sum($montosCuotasRedondeados) !== $totalRedondeado) {
            // Si no coinciden, puedes ajustar la última cuota para que la suma sea igual al total
            $diferencia = $totalRedondeado - array_sum($montosCuotasRedondeados);
            $ultimaCuotaIndex = count($montosCuotasRedondeados) - 1;
            $montosCuotasRedondeados[$ultimaCuotaIndex] += $diferencia;
        }


                // Ahora puedes continuar con la construcción del XML
        for ($i = 0; $i < count($montosCuotasRedondeados); $i++) {
          $facturaXML .= '<cac:PaymentTerms>
              <cbc:ID>FormaPago</cbc:ID>
              <cbc:PaymentMeansID>Cuota' . $ncuotacredito[$i] . '</cbc:PaymentMeansID>
              <cbc:Amount currencyID="' . $monedaf[$i] . '">' . $montosCuotasRedondeados[$i] . '</cbc:Amount>
    ...
    $sqlDetalle = "update factura set DetalleSunat='XML Firmado Correctamente' , hashc='$data[0]', estado='4' where idfactura='$idfactura'";
     ...
     factura f inner join persona p on f.idcliente=p.idpersona inner join empresa e on f.idempresa=e.idempresa where
      ...
      while ($row = mysqli_fetch_assoc($result)) {
        ...
        try {
          ...
          $sqlCodigo = "update factura set CodigoRptaSunat='', DetalleSunat='Error no se envio corregir' where idfactura='$idfactura'";
          ejecutarConsulta($sqlCodigo);
        }
  ...
  where
      date(f.fecha_emision_01) = '$fecha_actual' and e.idempresa = '$idempresa'
  ...
  public function listarValidar($ano, $mes, $dia, $idempresa, $idusuario)
  {
...
as tabla where year(fecha)='$ano' and month(fecha)='$mes' and day(fecha)='$dia' and idempresa='$idempresa'";
  ...
  public function listarDR($ano, $mes, $idempresa, $idusuario)
  {
    $sql = "
    SELECT 
    idfactura,
...
ORDER BY idfactura DESC";
  ...
  public function ventadetalle($idfactura)
  {
    $sql = "select
        ...
        um.abre as umedidacompra,
        dfa.numero_orden_item_33 as norden

        from
        detalle_fac_art dfa inner join articulo a on dfa.idarticulo=a.idarticulo inner join umedida um on a.umedidacompra=um.idunidad
        where
