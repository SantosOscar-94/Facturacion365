   ...
   public function insertar($idarticulo, $idalmacen, $codigo_proveedor, $codigo, $nombre, $idfamilia, $unidad_medida, $costo_compra, $saldo_iniu, $valor_iniu, $saldo_finu, $valor_finu, $stock, $comprast, $ventast, $portador, $merma, $precio_venta, $imagen, $codigosunat, $ccontable, $precio2, $precio3, $cicbper, $nticbperi, $ctticbperi, $mticbperu, $codigott, $desctt, $codigointtt, $nombrett, $lote, $marca, $fechafabricacion, $fechavencimiento, $procedencia, $fabricante, $registrosanitario, $fechaingalm, $fechafinalma, $proveedor, $seriefaccompra, $numerofaccompra, $fechafacturacompra, $limitestock, $tipoitem, $umedidacompra, $factorc, $descripcion)
{
    ...
    $sql = "INSERT INTO articulo (
            ...
            valor_fin_kardex, precio_final_kardex, fecharegistro, codigosunat, ccontable, 
            precio2, precio3, cicbper, nticbperi, ctticbperi, mticbperu, 
            codigott, desctt, codigointtt, nombrett, lote, marca, 
            fechafabricacion, fechavencimiento, procedencia, fabricante, registrosanitario, 
            fechaingalm, fechafinalma, proveedor, seriefaccompra, numerofaccompra, 
            fechafacturacompra, limitestock, tipoitem, umedidacompra, factorc, descrip
        ) 
        VALUES (
            '$idalmacen', '$codigo_proveedor','$codigo','$nombre','$idfamilia','$unidad_medida',
            '$costo_compra','$saldo_iniu','$valor_iniu','$saldo_finu','$valor_finu','$stock',
            '$comprast','$ventast','$portador','$merma','$precio_venta','$imagen',
            '$valor_finu','$costo_compra', NOW(), '$codigosunat', '$ccontable',
            '$precio2', '$precio3', '$cicbper','$nticbperi','$ctticbperi','$mticbperu',
            '$codigott', '$desctt', '$codigointtt', '$nombrett', '$lote', '$marca',
            $fechafabricacion, $fechavencimiento, '$procedencia', '$fabricante', '$registrosanitario',
            $fechaingalm, $fechafinalma, '$proveedor', '$seriefaccompra', '$numerofaccompra',
    ...
    $sqlreginv = "INSERT INTO reginventariosanos (
        ...
        '$comprast', '$ventast', '$saldo_finu', '$costo_compra', '$valor_finu', YEAR(CURDATE())
    )";
    ...
    $sqlsubarti = "INSERT INTO subarticulo (
        ...
        '$idartinew', '$codigo', '$costo_compra', '$costo_compra', '$stock', '$unidad_medida', '1'
    )";
    ...
    public function articuloBusqueda($codigo, $idempresa)
    {

        $sql = "select nombre, stock, precio_venta, um.abre as unidad_medida
        from articulo a
        inner join umedida um on a.unidad_medida = um.idunidad
        where codigo = '$codigo' and a.idempresa = '$idempresa';";
    ...
    public function listar($idempresa, $idalmacen)
    {
        ...
        al.nombre as nombreal
            ...
            inner join 
                empresa e on al.idempresa = e.idempresa 
            inner join 
                umedida um on a.umedidacompra = um.idunidad and a.tipoitem = 'productos'
            where 
        not a.nombre = '1000ncdg' 
        and e.idempresa = '$idempresa' 
        and al.idalmacen = '$idalmacen'
        and al.estado = '1'";
...
format(((sum(mg.totalventas) - sum(mg.totalcompras))/ sum(mg.totalventas)) * 100,2) as porcentaje from  margenganancia mg inner join articulo a on mg.idarticulo=a.idarticulo where  mg.ano='$ano' and mg.mes='$mes' group by a.nombre ";
    ...
    public function GenerarCodigoCorrelativoAutomatico($codigoExistente = null, $idempresa)
    {
        global $conexion;
        ...
        $sql = "select max(codigo) as last_code from articulo where codigo like 'pr%' and idempresa = '$idempresa'";
