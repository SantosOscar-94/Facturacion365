...
require_once "../modelos/ModeloTransferencia.php";
require_once "../modelos/Numeracion.php";
$modelotransferencia = new ModeloTransferencia();
...
switch ($_GET["op"]) {
    ...
    case 'guardaryeditarCambioestado':
        $rspta = $modelotransferencia->actualizarestados($_POST["idnota"], $cestado1);
    ...
    case 'guardaryeditarBoleta':
        ...
        if (empty($idnotaped)) {
            ...
            if ($idcliente == "N") {
                ...
                $rspta = $modelotransferencia->insertar(
                    $idusuario,
                    $fecha_emision_01,
                    $firma_digital_36,
                    $idempresa,
    ...
    case 'anular':
        $rspta = $modelotransferencia->anular($idnotaped);
    ...
    case 'baja':
        ...
        $hor = $_GET['hora'];
        $hoy = date("Y-m-d");
        $rspta = $modelotransferencia->baja($idboleta, $hoy, $com, $hor);
    ...
    case 'mostrar':
        $rspta = $modelotransferencia->mostrar($idnotaped);
    ...
    case 'listarDetalle':
        ...
        $rspta = $modelotransferencia->listarDetalle($id);
        $subt = 0;
        $igv = 0;
        $total = 0;
        echo '
    ...
    case 'selectSerie':
        ...
        $rspta = $numeracion->llenarSerieNtransferenciastock($idusuario);
    ...
    case 'listar':
        $rspta = $modelotransferencia->listar();
        //Vamos a declarar un array
        $data = array();

        while ($reg = $rspta->fetch_object()) {

            $urlT = '../reportes/exTransferenciaTicket.php?id=';
            // $urlB = '../reportes/exNotapedido.php?id=';
            // $urlC = '../reportes/exNotapedidocompleto.php?id=';

            if ($reg->tipo_documento_06 == 'Ticket') {
                $url = '../reportes/exTransferenciaTicket.php?id=';
            } else {
    ...
    case 'autonumeracion':
        ...
        $numeracion = new Numeracion();
        $Ser = $_GET['ser'];
        $rspta = $numeracion->llenarNumeroNtransferenciastock($Ser);
        while ($reg = $rspta->fetch_object()) {
            echo $reg->Nnumero;
        }
        break;
    ...
    case 'enviarcorreo':
        ...
        $rspta = $modelotransferencia->enviarcorreo($idnotaped);
        echo $rspta;
        break;
    ...
    case 'listarcomprobantesclientesEstado':
        ...
        $rspta = $modelotransferencia->listarcomprobantes($dnicliente);
        $data = array();
        while ($reg = $rspta->fetch_object()) {
            $data[] = array(
    ...
    case 'listarcomprobantes':
        $rspta = $modelotransferencia->listarcomprobantesCE();
        $data = array();
        while ($reg = $rspta->fetch_object()) {
            $data[] = array(
    ...
    case 'selectAlmacen':
        ...
        $rspta = $modelotransferencia->almacenlista();
        while ($reg = $rspta->fetch_object()) {
    ...
    case 'mostrarultimocomprobanteId':
        $rspta = $modelotransferencia->mostrarultimocomprobanteId($_SESSION['idempresa']);
        echo json_encode($rspta);
        break;
}
