...
if ($action == 'listarAlmacen') {
    ...
    $rspta = $modelostock->totalAlmacenActiva($nombre, $estado, $idempresa);
    $data = array();
...
if (isset($_GET["op"])) {
    ...
    switch ($_GET["op"]) {
        ...
        case 'listarStockProductos':
            ...
            while ($reg = $rspta->fetch_object()) {
                $imagenUrl = (empty($reg->imagen) || !file_exists("../files/articulos/" . $reg->imagen)) 
                    ? '../files/articulos/simagen.png' 
