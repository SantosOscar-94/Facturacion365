...
$pdf->Cell(40,6,'',0,0,'C');
$pdf->Cell(100,2,utf8_decode('REPORTE DE EGRESOS DEL DÍA'),0,0,'C'); 
...
$rspta = $insumos->categoriaagrupadogastos($_POST['fechagasto']);
while($reg0= $rspta->fetch_object())
{
	...
	$pdf->SetFont('COURIER','B',8);
		$rspta2 = $insumos->categoriaagrupadogastosDet($categoria, $_POST['fechagasto']);
		$pdf->Row(array(utf8_decode('Descripción'), 'acredor', 'Precio'));
	 ...
	 while($reg1= $rspta2->fetch_object())
	{
    	...
    	$pre = $reg1->totalgd;
    	$pdf->Row(array($descri, $acredor, $pre));
	}
	

	//=====================================
    	$rspta3 = $insumos->totalxcategoria($categoria, $_POST['fechagasto']);
		...
		$rspta4 = $insumos->totalxcategoriageneral($_POST['fechagasto']);
    	while($reg3= $rspta4->fetch_object())
		{
		$pdf->SetFont('COURIER','B',10);
		$totalgeneral = $reg3->totalxcategeneral;
		
		$pdf->Row(array('Total general gastos',$totalgeneral));
    	} 
$pdf->Output();
}
else
